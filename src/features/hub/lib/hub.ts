...
export const hubUrl = {
  ...
  // 条件（働き方）軸。職種を横断するためカテゴリの下には置けない。
  feature: (featureSlug: string) => `/jobs/feature/${featureSlug}`,
}

/**
 * 条件（働き方）ハブ。職種ではなく「どう働くか」で切る面。
 *
 * ルート配送は 配送・宅配3,260件＋トラック472件と**職種をまたぐ**ため、既存の職種ハブでは
 * 受け皿にならない。競合（ドラEVER）が「ルート配送 求人」(SV2,400)・「やめとけ」(880)・
 * 「きつい」(480)・「ホワイト企業」(390) のいずれでも10位圏外＝当社の勝ち目が最も大きい面。
 *
 * ⚠️ 増やすときは在庫と検索需要の両方を確認する。需要の無い条件を並べると薄いページの量産になる。
 */
export interface HubFeature {
  slug: string
  name: string
  /** title/description のどちらかに含まれれば該当とみなす語。 */
  match: string[]
  lead: string
  body: string
}
...
export const HUB_FEATURES: HubFeature[] = [
  {
    ...
    match: ["ルート配送"],
    lead:
      "決まった配送先を決まった順番でまわるルート配送の求人をまとめました。" +
      "新規開拓の営業がなく、走る道と荷物を覚えれば1日の流れが読めるため、" +
      "未経験から入る人が多い働き方です。同じ「配送」でも宅配便や長距離とは中身が違います。",
    body: `<h2>ルート配送とはどんな働き方ですか？</h2>
<p>ルート配送は、取引先が決まっていて、同じ配送先を定期的にまわる仕事です。コンビニやスーパー、飲食店、工場、事業所などへ、決まった曜日・決まった時間帯に届けます。飛び込みで新しい届け先が増えることは基本的にありません。</p>
<p>宅配便との一番の違いは、<strong>届け先が毎日変わらない</strong>ことです。宅配は個人宅が相手で不在再配達が発生しますが、ルート配送の相手は事業所なので、受け取り手が必ずいます。長距離輸送との違いは、多くが日帰りで、朝出て夕方に戻る形が中心である点です。</p>

<h2>給与はどのくらいですか？</h2>
<p>当サイトに掲載中のルート配送求人のうち、月給が明示されている3,032件の<strong>中央値は230,000円</strong>です（下限額ベース・2026年8月時点）。参考までに、掲載中のトラックドライバー全体は248,687円、配送・宅配ドライバー全体は225,000円で、ルート配送はその中間にあたります。</p>
<p>長距離のように距離手当で伸びる働き方ではないぶん、額面の振れ幅は小さくなります。求人票を見るときは、基本給と固定残業代の内訳、そして積み降ろしが手作業かパレットかを確認してください。同じ月給でも一日の体力の使い方がまったく変わります。</p>

<h2>どんな雇用形態がありますか？</h2>
<p>掲載中3,733件の内訳は、正社員2,572件・パート701件・契約社員60件・その他400件です（2026年8月時点）。掲載求人は毎週入れ替わるため、最新の総数はページ上部の件数をご覧ください。<strong>パートの比率が高い</strong>のがルート配送の特徴で、午前だけ・午後だけといった短時間の募集が一定数あります。扶養の範囲で働きたい人や、他の仕事と組み合わせたい人の入口になっています。</p>

<h2>会社を選ぶときは何を見ればいいですか？</h2>
<p>ルート配送は会社によって当たり外れが大きい仕事です。求人票では次の4点を確認してください。</p>
<ul>
<li><strong>1日の配送件数と拘束時間</strong>。件数だけ書いて時間が書かれていない求人は、実態を聞いてから判断する</li>
<li><strong>荷物の中身と積み降ろしの方法</strong>。パレット・カゴ車ならフォークリフトや手押しで済み、バラ積みなら手作業になる</li>
<li><strong>担当ルートの決まり方</strong>。固定なのか、日によって変わるのか。固定のほうが1日の見通しは立てやすい</li>
<li><strong>付帯業務の有無</strong>。検品・品出し・伝票処理が含まれるかで、実際の労働時間が変わる</li>
</ul>
<p>より詳しい見極め方は<a href="https://ridejob.jp/media/blog/route-delivery-driver">ルート配送ドライバーはやめとけ？きつい理由と向いている人・会社の選び方</a>で整理しています。</p>`,
  },
]

export const findFeature = (slug: string): HubFeature | undefined =>
  HUB_FEATURES.find((f) => f.slug === slug)

/**
 * 職種ハブから条件ハブへの導線。新設した面は内部リンクが無いと発見されないため、
 * 在庫の主戦場になっている職種からリンクを張る。
 */
const FEATURES_BY_CAT: Record<string, string[]> = {
  "delivery-driver": ["route-delivery"],
  "truck-driver": ["route-delivery"],
}
export const featuresForCatSlug = (catSlug?: string): HubFeature[] =>
  (catSlug ? (FEATURES_BY_CAT[catSlug] ?? []) : []).flatMap((s) => findFeature(s) ?? [])
